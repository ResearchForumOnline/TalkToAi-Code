...
from updates import VERSION, REPO, check_release, download_release
...
class UpdateDialog(QDialog):
    def __init__(self, owner, state):
        ...
        self.status=QLabel('Updates are checked only when you ask. No AI tokens are used.'); self.status.setWordWrap(True); layout.addWidget(self.status)
        self.notes=QTextBrowser(); layout.addWidget(self.notes)
        self.check=QPushButton('Check GitHub for updates'); self.check.clicked.connect(lambda:self.work(check_release)); layout.addWidget(self.check)
    ...
    def completed(self, result):
        ...
        if isinstance(result, dict):
            ...
            self.download.setEnabled(result['newer'])
        else:
            self.installer=result; self.install.setEnabled(True)
