...
def render(path, width=1440, height=900, steps=False, jobs=False, providers=False):
    ...
    with tempfile.TemporaryDirectory() as folder, ExitStack() as stack:
        ...
        if jobs:
            window.handle_event('job',{'id':'demo-only','state':'completed','command':['python','-m','unittest'], 'cwd':'Demo project', 'seconds':1.2,'exit_code':0,'output':'UI DEMONSTRATION ONLY\n\nThis panel shows command output, exit status and cancellation.\nThis is illustrative text, not an executed test result.'})
            window.right.setCurrentIndex(window.jobs_tab)
            window.prompt.clear()
            window.use_starter('Build and collect outputs')
        window.show();app.processEvents()
        target=Path(path);target.parent.mkdir(parents=True,exist_ok=True)
        if providers:
            from provider_dialog import ProviderDialog
            dialog=ProviderDialog(window,root/'providers.json');dialog.preset.setCurrentIndex(1);dialog.show();app.processEvents()
            if not dialog.grab().save(str(target)):raise RuntimeError('Provider preview rendering failed')
            dialog.close();dialog.deleteLater();app.processEvents()
        elif not window.grab().save(str(target)):raise RuntimeError('Preview rendering failed')
        window.allow_quit=True;window.close();window.deleteLater();app.processEvents()


if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('path');parser.add_argument('--width',type=int,default=1440);parser.add_argument('--height',type=int,default=900);parser.add_argument('--steps',action='store_true');parser.add_argument('--jobs',action='store_true');parser.add_argument('--providers',action='store_true')
    args=parser.parse_args();render(args.path,args.width,args.height,args.steps,args.jobs,args.providers)
