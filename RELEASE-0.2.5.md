...
The installer is built with the existing standard Windows x64 Inno Setup configuration. The packaged runtime is tested separately before publication. The user's running 0.2.4 session is deliberately left untouched; an in-place 0.2.5 upgrade on that active installation is not claimed as tested.
